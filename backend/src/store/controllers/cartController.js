...
const viewCart = (req, res) => {
    ...
    pool.query(`SELECT i.id, i.item_name, i.price, ci.quantity, (i.price * ci.quantity) as total_item_price
    FROM items i
...
const addToCart = async (req, res) => {
    ...
    try {
        ...
        console.log(usersCartId, id, quantity);
        ...
        if (isItemInCart.rows.length === 0 ) {
            ...
            let wholeQuantity = existingQuantity.rows[0].quantity + quantity;
        ...
        console.error(err);
        return res.status(500).json({"error": "Internal server errordfsdfds"})
    }
